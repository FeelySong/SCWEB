...
if (strpos($_SESSION['admin_flag'],'25') ){}else{ 
...
if($_GET['act']=="dels"){       //批量删除   
	...
	$sql="delete from ssc_memberamend where id in ($ids)";  
	mysql_query($sql);  
}

if($_GET['act']=="del"){
	mysql_query("Delete from ssc_memberamend where id=".$_GET['id']);
	echo "<script>window.location.href='mem_amend.php';</script>"; 
	exit;
}
...
$urls="starttime=".$starttime."&endtime=".$endtime."&username=".$username_mem."&level=".$level."&ip=".$ip;

$s1=$s1." order by id desc";
$sql="select * from ssc_memberamend where 1=1".$s1;

$rs = mysql_query($sql);
$total = mysql_num_rows($rs);

$sql="select * from ssc_memberamend where 1=1".$s1." limit $page2,$pagesize";
...
<br>
	<table width="95%" border="0" cellpadding="0" cellspacing="0" class="top_list">
      <tr>
        <td class="top_list_td icons_a2">　　您现在的位置是：会员管理 &gt; 操作日志</td>
      </tr>
    </table>
<br>
    ...
    <table width="95%" border="0" cellpadding="0" cellspacing="0">
      <tr>
        <td class="top_list_td">
          ...
          <input name="username" type="text" class="inpa" value="" size="20" maxlength="30" id="username">
          IP地址:
          
          <input name="ip" type="text" class="inpa" value="" size="20" maxlength="30" id="ip">
&nbsp;
        <input name="Find_VN" type="submit" class="btnb" value="搜 索" style="position: relative; top: 2px;"></td>
        <td width="10" class="top_list_td">&nbsp;</td>
      </tr>
    </table>
    </form>
<br>
	...
	<table width="95%" border="1" cellpadding="0" cellspacing="0" bordercolor="#DDDDDD" class="t_list">
        <tr>
          ...
          <td class="t_list_caption">用户名</td>
            ...
            <td class="t_list_caption">操作内容</td>
		...
		?>
            <tr class="t_list_tr_0" onMouseOver="this.style.backgroundColor='#FFFFA2'" onMouseOut="this.style.backgroundColor=''">
                ...
                <td><?=$row['cont']?></td>
                <td><?=$row['adddate']?></td>
                <td><a href="?uname=<?=$row['username']."&".$urls?>">只看此人</a> <a href="?act=del&id=<?=$row['id']?>" onClick="return confirm('确认要删除吗?');">删除</a></td>
      </tr>
 		<?php
		}
		?>
