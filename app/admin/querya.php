...
//			echo $t1.$t2.$t3."<br>";
	...
	}else if($row['id']==11){//cq115
//		echo "dd";
//		$content=file_get_contents($row['port']);
//		$stra=split("fc333",$content);
//		echo $stra[1];
	}else{
		$doc = new DOMDocument();
		$doc->load($row['port']); //读取xml文件
...
//				echo $row['name']."第".$t1."期:".$t2."<br>";
				...
				autokj($n1,$n2,$n3,$n4,$n5,$cid,$t1,0);
			}
		}else{
			if($rowa['zt']=="0"){
				if($rowa['sign']>5){
					autokj($n1,$n2,$n3,$n4,$n5,$cid,$t1,1);
					$sqls="update ssc_data set zt=1 where id ='".$rowa['id']."'";
