...
if (strpos($_SESSION['admin_flag'],'61') ){}else{ 
...
if ($cid==2){ 
	$sql="select DATE_FORMAT( adddate, '%Y-%m' ) AS t0,SUM(IF(types = 1, smoney, 0)) as t1,SUM(IF(types = 2, zmoney, 0)) as t2,SUM(IF(types = 3, smoney, 0)) as t3,SUM(IF(types = 7, zmoney, 0)) as t7,SUM(IF(types = 11, smoney, 0)) as t11,SUM(IF(types = 12, smoney, 0)) as t12,SUM(IF(types = 13, smoney, 0)) as t13,SUM(IF(types = 15, zmoney, 0)) as t15,SUM(IF(types = 16, zmoney, 0)) as t16,SUM(IF(types = 32, smoney, 0)) as t32,SUM(IF(types = 40, smoney, 0)) as t40 from ssc_record where 1=1 ".$s1." GROUP BY DATE_FORMAT( adddate, '%Y-%m' ) order by t0 desc limit $page2,$pagesize";
}else{
	$sql="select DATE_FORMAT( adddate, '%Y-%m-%d' ) AS t0,SUM(IF(types = 1, smoney, 0)) as t1,SUM(IF(types = 2, zmoney, 0)) as t2,SUM(IF(types = 3, smoney, 0)) as t3,SUM(IF(types = 7, zmoney, 0)) as t7,SUM(IF(types = 11, smoney, 0)) as t11,SUM(IF(types = 12, smoney, 0)) as t12,SUM(IF(types = 13, smoney, 0)) as t13,SUM(IF(types = 15, zmoney, 0)) as t15,SUM(IF(types = 16, zmoney, 0)) as t16,SUM(IF(types = 32, smoney, 0)) as t32,SUM(IF(types = 40, smoney, 0)) as t40 from ssc_record where 1=1 ".$s1." GROUP BY DATE_FORMAT( adddate, '%Y-%m-%d' ) order by t0 desc limit $page2,$pagesize";
}
...
<div align="center"><br />
	<table width="95%" border="0" cellpadding="0" cellspacing="0" class="top_list">
      <tr>
        <td class="top_list_td icons_a6">　　您现在的位置是：数据统计 &gt; 结算报表</td>
      </tr>
    </table>
  	<br />
    ...
    <table width="95%" border="0" cellpadding="0" cellspacing="0">
      <tr>
        <td align="left" class="top_list_td">结算时间：
