...
if (strpos($_SESSION['admin_flag'],'a3') ){}else{ 
...
if($_GET['act']=="dels"){       //批量删除   
	...
	$sql="delete from ssc_manageramend where id in ($ids)";  
	mysql_query($sql);  
}

if($_GET['act']=="del"){
	mysql_query("Delete from ssc_manageramend where id=".$_GET['id']);
	echo "<script>window.location.href='admin_amend.php';</script>"; 
	exit;
}
...
$urls="starttime=".$starttime."&endtime=".$endtime."&username=".$username_mem."&level=".$level;

$s1=$s1." order by id desc";
$sql="select * from ssc_manageramend where 1=1".$s1;

$rs = mysql_query($sql);
$total = mysql_num_rows($rs);

$sql="select * from ssc_manageramend where 1=1".$s1." limit $page2,$pagesize";
...
<br>
	<table width="95%" border="0" cellpadding="0" cellspacing="0" class="top_list">
      <tr>
        <td class="top_list_td icons_a10">　　您现在的位置是：权限管理 &gt; 操作日志</td>
      </tr>
    </table>
<br>
...
<tr>
          ...
          <td class="t_list_caption">用户名</td>
            ...
            <td class="t_list_caption">所在位置</td>
