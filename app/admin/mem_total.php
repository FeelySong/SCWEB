...
if (strpos($_SESSION['admin_flag'],'26') ){}else{ 
...
if ($cid==1){ 
	$sql = "SELECT DATE_FORMAT( regdate, '%Y-%m-%d' ) AS numa FROM ssc_member GROUP BY DATE_FORMAT( regdate, '%Y-%m-%d' )";
}
if ($cid==2){ 
	$sql = "SELECT DATE_FORMAT( regdate, '%Y-%m' ) AS numa FROM ssc_member GROUP BY DATE_FORMAT( regdate, '%Y-%m' )";
}
if ($cid==3){ 
	$sql = "SELECT DATE_FORMAT( regdate, '%Y' ) AS numa FROM ssc_member GROUP BY DATE_FORMAT( regdate, '%Y' )";
}
$rs = mysql_query($sql);
$total = mysql_num_rows($rs);

if ($cid==1){ 
	$sql = "SELECT DATE_FORMAT( regdate, '%Y-%m-%d' ) AS numa, count(IF(level = 0, level, null)) as numc, count(IF(level = 1, level, null)) as numd, count(IF(level = 2, level, null)) as nume FROM ssc_member GROUP BY DATE_FORMAT( regdate, '%Y-%m-%d' ) order by numa desc limit $page2,$pagesize";
}
if ($cid==2){ 
	$sql = "SELECT DATE_FORMAT( regdate, '%Y-%m' ) AS numa, count(IF(level = 0, level, null)) as numc, count(IF(level = 1, level, null)) as numd, count(IF(level = 2, level, null)) as nume FROM ssc_member GROUP BY DATE_FORMAT( regdate, '%Y-%m' ) order by numa desc limit $page2,$pagesize";
}
if ($cid==3){ 
	$sql = "SELECT DATE_FORMAT( regdate, '%Y' ) AS numa, count(IF(level = 0, level, null)) as numc, count(IF(level = 1, level, null)) as numd, count(IF(level = 2, level, null)) as nume FROM ssc_member GROUP BY DATE_FORMAT( regdate, '%Y' ) order by numa desc limit $page2,$pagesize";
}
...
<div align="center"><br />
	<table width="95%" border="0" cellpadding="0" cellspacing="0" class="top_list">
      <tr>
        <td class="top_list_td icons_a2">　　您现在的位置是：会员管理 &gt; 用户统计 &gt; <?php if($cid==1){echo "按日统计";}else if($cid==2){echo "按月统计";}else if($cid==3){echo "按年统计";}?></td>
      </tr>
    </table>
  	<br />
