...
if (strpos($_SESSION['admin_flag'],'67') ){}else{ 
...
if ($cid==2){ 
	$sql="select DATE_FORMAT( adddate, '%Y-%m' ) AS t0,SUM(IF(types = 40, smoney, 0)) as t40 from ssc_record where 1=1 ".$s1." GROUP BY DATE_FORMAT( adddate, '%Y-%m' ) order by t0 desc limit $page2,$pagesize";
}else{
	$sql="select DATE_FORMAT( adddate, '%Y-%m-%d' ) AS t0,SUM(IF(types = 40, smoney, 0)) as t40 from ssc_record where 1=1 ".$s1." GROUP BY DATE_FORMAT( adddate, '%Y-%m-%d' ) order by t0 desc limit $page2,$pagesize";
}
...
<div align="center"><br />
	<table width="95%" border="0" cellpadding="0" cellspacing="0" class="top_list">
      <tr>
        <td class="top_list_td icons_a6">　　您现在的位置是：数据统计 &gt; 分红统计</td>
      </tr>
    </table>
  	<br />
