...
if (strpos($_SESSION['admin_flag'],'76') ){}else{ 
...
if($act=="edit"){
	...
	$sql="update ssc_class set maxbei = '".$_POST['maxbei']."' where id ='".$_POST['sid']."'";
//	echo $sql;
	if (!mysql_query($sql)){
...
$sql="select * from ssc_class where cid='".$cid."' order by id asc";
...
<div align="center"><br />
	<table width="95%" border="0" cellpadding="0" cellspacing="0" class="top_list">
      <tr>
        <td class="top_list_td icons_a7">　　您现在的位置是：游戏管理 &gt; 限倍管理</td>
      </tr>
    </table>
  	<br />
...
<table width="95%" border="1" cellpadding="0" cellspacing="0" bordercolor="#DDDDDD" class="t_list">
			  ...
			  <td class="t_list_caption">最小倍数</td>
	          <td class="t_list_caption">最大倍数</td>
			...
			<tr class="t_list_tr_<?=$i%2?>" onMouseOver="this.style.backgroundColor='#E8F1FF'" onMouseOut="this.style.backgroundColor=''">
				...
				<td align="center"><input name="minbei" type="text" class="inp1" id="minbei" onfocus="this.className='inp1a'" onblur="this.className='inp1';" onkeypress="alphaOnly(event);" value="<?=$row['minbei']?>" size="15" maxlength="15" /></td>
			    <td align="center"><input name="maxbei" type="text" class="inp1" id="maxbei" onfocus="this.className='inp1a'" onblur="this.className='inp1';" onkeypress="alphaOnly(event);" value="<?=$row['maxbei']?>" size="15" maxlength="15" /></td>
