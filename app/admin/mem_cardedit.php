...
if($_GET['act']=="edit"){
	$sql="select * from ssc_bankcard where id= ".$_GET['id'];
...
<br />
	<table width="95%" border="0" cellpadding="0" cellspacing="0" class="top_list">
      <tr>
        <td class="top_list_td icons_a2">　　您现在的位置是：会员管理 &gt; <?php if($_GET['act']=="edit"){echo "编辑银行卡信息";}?></td>
      </tr>
    </table>
<br>
<form name="sonMemberForm" method="post" action="mem_card.php?act=<?=$_GET['act']?>&id=<?=$_GET['id']?>" onSubmit="return SubChk()">
<table width="95%" border="1" cellpadding="0" cellspacing="0" bordercolor="#DDDDDD" class="t_list">
    <tr>
        <td width="150" height="40" class="t_Edit_caption">用户名</td>
        <td class="t_Edit_td"><?=$rs['username']?></td>
    </tr>
    <tr>
      <td height="40" class="t_Edit_caption">开户银行</td>
      <td class="t_Edit_td"><select name="bank" id="bank" style='width:170px;'>
<?php
$sqla = "select * from ssc_banks order by id asc";
$rsa = mysql_query($sqla);
while ($rowa = mysql_fetch_array($rsa)){
?>
    	<option value="<?=$rowa['id']?>" <?php if($rowa['name']==$rs['bankname']){echo "selected";}?>><?=$rowa['name']?></option>
<?php }?>
    	</select>
		</td>
    </tr>
    <tr>
      <td height="40" class="t_Edit_caption">开户省市</td>
      <td class="t_Edit_td"><select name="province" id="province" onchange="changeCity();" style='width:170px;'>
        <option value="<?=$rs['province']?>"><?=Get_province($rs['province'])?></option>
        ...
        <select name="city" id="city"  style='width:170px;'>
          <option value="<?=$rs['city']?>"><?=Get_city($rs['city'])?></option>
        </select>
      </td>
    </tr>
    <tr>
      <td height="40" class="t_Edit_caption">支行名称</td>
      <td class="t_Edit_td"><input name="bankbranch" type="text" class="inp2" id="bankbranch" onFocus="this.className='inp2a'" onBlur="this.className='inp2';" value="<?=$rs['bankbranch']?>" size="30">
      <span class="helpinfo">( 由2至50个字符组成 )</span></td>
    </tr>
    <tr>
      <td height="40" class="t_Edit_caption">开户姓名</td>
      <td class="t_Edit_td"><input name="realname" type="text" class="inp2" id="realname" onFocus="this.className='inp2a'" onBlur="this.className='inp2';" value="<?=$rs['realname']?>" size="30">
      <span class="helpinfo">( 由2至8个字符组成 )</span></td>
    </tr>
    <tr>
      <td height="40" class="t_Edit_caption">银行帐号</td>
      <td class="t_Edit_td"><input name="cardno" type="text" class="inp2" id="cardno" onFocus="this.className='inp2a'" onBlur="this.className='inp2';" value="<?=$rs['cardno']?>" size="30">
      (16-19位银行帐号)</td>
    </tr>
  </table>
<table width="500">
