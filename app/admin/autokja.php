...
//			
			...
			//xc添加
			if($mid=="32" || $mid=="70" || $mid=="108" || $mid=="146" || $mid==334){//万位
				$nums=0;
				$strb=explode("&",$row['codes']);
				for ($ii=0; $ii<count($strb); $ii++) {
					if($strb[$ii]==$na[0]){$nums=$nums+1;}
				}
			}else if($mid=="33" || $mid=="71" || $mid=="109" || $mid=="147" || $mid==335){//千位
				$nums=0;
				$strb=explode("&",$row['codes']);
				for ($ii=0; $ii<count($strb); $ii++) {
					if($strb[$ii]==$na[1]){$nums=$nums+1;}
				}
			}else if($mid=="34" || $mid=="72" || $mid=="110" || $mid=="148" || $mid==336 || $mid==176 || $mid==306){//百位
				$nums=0;
				$strb=explode("&",$row['codes']);
				for ($ii=0; $ii<count($strb); $ii++) {
					if($strb[$ii]==$na[2]){$nums=$nums+1;}
				}
			}else if($mid=="35" || $mid=="73" || $mid=="111" || $mid=="149" || $mid==337 || $mid==177 || $mid==307){//十位
				$nums=0;
				$strb=explode("&",$row['codes']);
				for ($ii=0; $ii<count($strb); $ii++) {
					if($strb[$ii]==$na[3]){$nums=$nums+1;}
				}
			}else if($mid=="36" || $mid=="74" || $mid=="112" || $mid=="150" || $mid==338 || $mid==178 || $mid==308){//个位
				$nums=0;
				$strb=explode("&",$row['codes']);
				for ($ii=0; $ii<count($strb); $ii++) {
