...
if (!echo_validate_image($config))
{
    // 如果不成功则初始化一个默认验证码
    @session_start();
    $_SESSION['avalicode'] = strtolower('3825');
...
function echo_validate_image( $config = array() )
{
    ...
    $_SESSION['avalicode'] = strtolower($rndstring);
    ...
    for($i=0;$i<$rndcodelen;$i++)
    {
        ...
        $y_pos = $i==0 ? 6 : $i*($font_size+2)+6;
        $c = mt_rand(0, 15);
//h 19改 15
        @imagettftext($im, $font_size, $c, $y_pos, 20, $c_fontColor, $font_file, $rndstring[$i]);
        $lastc = $rndstring[$i];
    }
