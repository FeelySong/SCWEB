...
$sql_ph = "select * from ssc_record where types='12' and  adddate like '%".date("Y-m-d")."%' group by uid";		
$rs_ph = mysql_query($sql_ph);
while($row_ph= @mysql_fetch_array($rs_ph)){
	$sql_tj = "select sum(smoney) as tj from ssc_record where uid='".$row_ph['uid']."' and types='12' and  adddate like '%".date("Y-m-d")."%'";		
	$rs_tj = mysql_query($sql_tj);
	$row_tj= @mysql_fetch_array($rs_tj);	
	$sql_jc = "select * from ssc_paihang where uid='".$row_ph['uid']."'";		
	$rs_jc = mysql_query($sql_jc);
	$row_jc= @mysql_fetch_array($rs_jc);	
	if($row_jc['id']){
		mysql_query("update ssc_paihang set jiangjin='".$row_tj['tj']."',adddate='".time()."' where id='".$row_jc['id']."'");
	}else{
		mysql_query("INSERT INTO `ssc_paihang` (`username` ,`uid` ,`jiangjin` ,`adddate` )VALUES ( '".$row_ph['username']."', '".$row_ph['uid']."', '".$row_tj['tj']."', '".time()."')");
	}
};
?>
...
<H1><SPAN class="action-span zhuangtai"><A href="paihang.php
" target='_self'>玩家排行榜</a></SPAN>
<SPAN class="action-span1"><DIV class='tubiao'></DIV>当前位置: <A href="/" target='_top'><?php echo $webname;?></a>
 - 玩家排行榜 </SPAN><DIV style="clear:both"></DIV></H1>
<STYLE>
...
$i=0;
	$sql_ph = "select * from ssc_paihang where adddate>'$today' order by jiangjin desc limit 0,50";		
	$rs_ph = mysql_query($sql_ph);
	while($row_ph= @mysql_fetch_array($rs_ph)){	
$i++;
?>	  
      <tr class="needchangebg">
        ...
        <td align="center"><?=$row_ph['username']?></td>
        <td align="center"><?=$row_ph['jiangjin']?></td>
      </tr>	  
<?php }?>    
	</table>	
	</div>
