...
if($lottery!="" && $lottery!="0"){
	$s1=$s1." and lotteryid='".$lottery."'";
}
if($method!="" && $method!="0"){
	$s1=$s1." and mid='".$method."'";
}
...
$sql="select * from ssc_member where 1=1".$s2;
$rs = mysql_query($sql);
$total = mysql_num_rows($rs);

$sqlb="select * from ssc_member where regup='".$_SESSION["username"] ."' order by id asc";
...
<SPAN class="action-span zhuangtai"><A href="report_game.php" target='_self'>报表查询</a></SPAN>
<SPAN class="action-span1"><DIV class='icons_mb1'></DIV>当前位置: <A href="/" target='_top'><?php echo $webname;?></a>
 - 报表查询 </SPAN><DIV style="clear:both"></DIV></H1>
...
<div id="tabbar-div">
  ...
  <tr>
    <td width="86%">报表查询</td>
    <td width="14%" align="center"><SPAN class="action-span"><A href="./report_game.php" target='_self' style="color:#000; text-decoration:none;">清空查询条件</a></SPAN></td>
  </tr>
</table>
...
<tr>
	...
	<option value="0">所有游戏</option>
        ...
        <option value="100">金鲨银鲨</option>
		</select>
	&nbsp;&nbsp;&nbsp;
	<span id="method_show">游戏玩法: <select name='method' id='method' style='width:100px;'><option value='0' selected="selected">所有玩法</option></select></span>
	&nbsp;&nbsp;&nbsp;
...
?> 
	    <option value="<?=$rowb['username']?>"><?=$rowb['username']?></option>
<?php }?>
        </select>
...
<table class="lt" border="0" cellspacing="0" cellpadding="0">
	<tr class='th'>
        ...
        <td><div class='line'></div>总代购费</td>
        <td><div class='line'></div>返点</td>
        <td><div class='line'></div>实际总代购费</td>
        <td><div class='line'></div>中奖金额</td>
        <td><div class='line'></div>总结算</td>
...
<?php
	...
	$sqla="select sum(zmoney) as sgmoney from ssc_record where types='7' and username='".$row['username']."'".$s1;
	$rsa = mysql_query($sqla);
	$rowa = mysql_fetch_array($rsa);
	$gmoney=$rowa['sgmoney'];
	$sqla="select sum(smoney) as sfmoney from ssc_record where types='11' and username='".$row['username']."'".$s1;
	$rsa = mysql_query($sqla);
	$rowa = mysql_fetch_array($rsa);
	$fmoney=$rowa['sfmoney'];
	$sqla="select sum(smoney) as swmoney from ssc_record where types='12' and username='".$row['username']."'".$s1;
	$rsa = mysql_query($sqla);
	$rowa = mysql_fetch_array($rsa);
	$wmoney=$rowa['swmoney'];
...
?>
        <tr align="center">
        <td <?php if($row['username']==$uid || $row['username']==$_SESSION["username"] ){echo "style='color:red;'";}?>><?=$row['username']?></td>
        <td <?php if($row['username']==$uid || $row['username']==$_SESSION["username"] ){echo "style='color:red;'";}?>><?php if($row['level']==1){echo "代理";}else{echo "用户";}?></td>
        <td <?php if($row['username']==$uid || $row['username']==$_SESSION["username"] ){echo "style='color:red;'";}?>><?=number_format($gmoney,4)?></td>
        <td <?php if($row['username']==$uid || $row['username']==$_SESSION["username"] ){echo "style='color:red;'";}?>><?=number_format($fmoney,4)?></td>
        <td <?php if($row['username']==$uid || $row['username']==$_SESSION["username"] ){echo "style='color:red;'";}?>><?=number_format($gmoney-$fmoney,4)?></td>
        <td <?php if($row['username']==$uid || $row['username']==$_SESSION["username"] ){echo "style='color:red;'";}?>><?=number_format($wmoney,4)?></td>
        <td <?php if($row['username']==$uid || $row['username']==$_SESSION["username"] ){echo "style='color:red;'";}?>><?=number_format($gmoney-$fmoney-$wmoney,4)?></td>
        <td><?php if($row['username']==$uid || $row['username']==$_SESSION["username"] ){echo "<font color='#D0D0D0'>-----</font>";}else{?><a href="./report_game.php?uid=<?=$row['username']?>&starttime=<?=$starttime?>&endtime=<?=$endtime?>&lottery=<?=$lottery?>&method=<?=$method?>&isrequery=1">查看下级</a>
        &nbsp;&nbsp;
        <a href="./report_detail.php?uid=<?=$row['username']?>&starttime=<?=$starttime?>&endtime=<?=$endtime?>" target="_blank">游戏明细</a><?php }?></td>
    </tr>
<?php }?>
    <tr align="center">
        <td colspan="2">合计</td>
        <td><?=number_format($tgmoney,4)?></td>
        <td><?=number_format($tfmoney,4)?></td>
        <td><?=number_format($tgmoney-$tfmoney,4)?></td>
        <td><?=number_format($twmoney,4)?></td>
        <td><?=number_format($tgmoney-$tfmoney-$twmoney,4)?></td>
        <td></td>
    </tr>
<?php }?>
</table>
