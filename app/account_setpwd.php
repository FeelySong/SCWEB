...
<HEAD><TITLE><?php echo $webname;?>  - 设置资金密码</TITLE>
...
<SPAN class="action-span1"><DIV class='icons_mb1'></DIV>当前位置: <A href="/" target='_top'><?php echo $webname;?></a>
 - 设置资金密码 </SPAN><DIV style="clear:both"></DIV></H1>
...
<input type="hidden" name="nextact" value="draw" />
  <tr>
  	<td class="narrow-label">输入资金密码:</td>
    <td>你还没有设置资金密码，请先设置<br/>
	<input type="password" name="secpass" value="" id="password"/><br />(由字母和数字组成6-16个字符；且必须包含数字和字母，不允许连续三位相同，不能和登陆密码相同)</td>
  </tr>
  <tr><td></td><td><input type="submit" name="submit" value="提交" class="button" />&nbsp;&nbsp;<input type="button" value="重置" onClick="this.form.reset()" class="button" /></td></tr>
</form>
</table>
</div>
