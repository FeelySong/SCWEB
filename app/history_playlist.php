...
$username_playlist=$_REQUEST['username'];
...
if($username_playlist!=""){
	$s1=$s1." and username='".$username_playlist."'";
}
...
$urls="starttime=".$starttime."&endtime=".$endtime."&status=".$statue."&lotteryid=".$lotteryid."&methodid=".$methodid."&issue=".$issue."&mode=".$mode."&projectno=".$projectno."&username=".$username_playlist."&ranges=".$ranges."&isgetdata=".$isgetdata;
...
<HEAD><TITLE><?php echo $webname;?>  - 投注记录</TITLE>
...
<H1>
<SPAN class="action-span"><A href="history_tasklist.php" target='_self'>追号记录</a></SPAN>
<SPAN class="action-span zhuangtai"><A href="history_playlist.php" target='_self'>投注记录</a></SPAN>
<SPAN class="action-span1"><DIV class='tubiao'></DIV>当前位置: <A href="/" target='_top'><?php echo $webname;?></a>
 - 投注记录 </SPAN><DIV style="clear:both"></DIV></H1>
...
<div id="tabbar-div">
  ...
  <tr>
    <td width="86%">投注记录</td>
    <td width="14%" align="center"><SPAN class="action-span"><A href="history_playlist.php" target='_self' style="color:#000; text-decoration:none;">清空查询条件</a></SPAN></td>
  </tr>
</table>
    ...
    <tr>
    	<td>
			...
			&nbsp;&nbsp;范围: <select name='ranges' id="ranges">
			  <OPTION  value="2">全部</OPTION><OPTION selected value="6">自己</OPTION><OPTION  value="3">直接下级</OPTION><OPTION  value="4">所有下级</OPTION></select>						&nbsp;&nbsp;<font color=#009900>温馨提示: 点击注单编号可以查看详细注单信息以及撤单.</font>
		</td>
     </tr>
</form>
</table></div>
...
<?php 
  ...
  ?>
    <tr align="center" class="<?php if($row['zt']==4 || $row['zt']==5 || $row['zt']==6){echo "cancelline";}else{echo "needchangebg";}?>" >
        <td height="37" <?php if($row['zt']==4 || $row['zt']==5 || $row['zt']==6){echo 'style="background-color:#CCCCCC;"';}?>><a href="javascript:"  title="查看投注详情" class="blue" rel="projectinfo"><?=$row['dan']?></a></td>
        ...
        <td class="codelist" <?php if($row['zt']==4 || $row['zt']==5 || $row['zt']==6){echo 'style="background-color:#CCCCCC;"';}?>><?php if($row['lotteryid']==100){ echo($kjbh_arr[$row['codes']]);}else{if(strlen(dcode($row['codes'],$row['lotteryid']))<30){echo dcode($row['codes'],$row['lotteryid']);}else{?><a href="javascript:" rel='morecode'>详细号码</a><input type="hidden" value="<?=dcode($row['codes'],$row['lotteryid'])?>"><?php }}?></td>
        <td <?php if($row['zt']==4 || $row['zt']==5 || $row['zt']==6){echo 'style="background-color:#CCCCCC;"';}?>><?=$row['times']?></td>
        <td <?php if($row['zt']==4 || $row['zt']==5 || $row['zt']==6){echo 'style="background-color:#CCCCCC;"';}?>><?php if($row['mode']=='1'){echo "元";}elseif($row['mode']=='2'){echo "角";}else{echo "分";}?></td>
        <td <?php if($row['zt']==4 || $row['zt']==5 || $row['zt']==6){echo 'style="background-color:#CCCCCC;"';}?>><?=$row['money']?></td>
        <td <?php if($row['zt']==4 || $row['zt']==5 || $row['zt']==6){echo 'style="background-color:#CCCCCC;"';}?>><?php if($row['zt']==0){echo "0.0000";}else{echo $row['prize'];}?></td>
        <td <?php if($row['zt']==4 || $row['zt']==5 || $row['zt']==6){echo 'style="background-color:#CCCCCC;"';}?>> <?php if($row['lotteryid']==100){ echo($kjbh_arr[$row['kjcode']]);}else{echo($row['kjcode']);}?> </td>
        <td <?php if($row['zt']==4 || $row['zt']==5 || $row['zt']==6){echo 'style="background-color:#CCCCCC;"';}?>><?php if($row['zt']==0){echo "未开奖";}else if($row['zt']==1){echo "<font color=#D90000>已派奖</font>";}else if($row['zt']==2){echo "<font color=#639300>未中奖</font>";}else if($row['zt']==4){echo "管理员撤单";}else if($row['zt']==5){echo "本人撤单";}else if($row['zt']==6){echo "开错奖撤单";}else if($row['zt']==7){echo "投注异常";}?></td>
    </tr>
  <?php }?>
        <tr align="center">
        <td height="37" bgcolor="#999999">小结</td>
        <td colspan="8" bgcolor="#999999">&nbsp;</td>
        <td bgcolor="#999999"><?=number_format($tmoney,4)?></td>
        <td bgcolor="#999999"><?=number_format($tprize,4)?></td>
