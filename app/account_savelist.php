...
$urls="time_min=".$time_min."&time_max=".$time_max."&check=952";
$s1=$s1." order by id desc";
$sql="select * from ssc_savelist where bank!='手动充值' and username='" . $_SESSION["username"] . "'".$s1;
//echo $sql;
$rs = mysql_query($sql);
$total = mysql_num_rows($rs);

$sql="select * from ssc_savelist where bank!='手动充值' and username='" . $_SESSION["username"] . "'".$s1." limit $page2,$pagesize";
...
<HEAD><TITLE><?php echo $webname;?>  - 充值记录</TITLE>
...
<H1><SPAN class="action-span zhuangtai"><A href="account_savelist.php?check=952" target='_self'>提现记录</a></SPAN><SPAN class="action-span"><A href="account_draw.php" target='_self'>平台提现</a></SPAN><SPAN class="action-span"><A href="account_autosave.php" target='_self'>自动充值</a></SPAN>
...
<div class="tab-div">
	<!--<div id="tabbar-div">
		...
		<span class="tab-back" id="general_tab_1">
		    <span class="tabbar-left"></span>
		    <span class="content" onclick="window.location.href='./account_save.php?check=952'">人工充值</span>
		    <span class="tabbar-right"></span>		</span>
	    <span class="tab-front" id="general_tab_2">
		    <span class="tabbar-left"></span>
		    <span class="content">充值记录</span>
...
<input type="hidden" name="check" value="952" />
<tr><td>
提现时间: <input type="text" size="16" name="time_min" id="time_min" value="<?=date("Y-m-d",strtotime("-2 week"))?> 02:20" /> 
...
<table class="lt" border="0" cellspacing="0" cellpadding="0">
	<tr class='th'>
		...
		<td><div class='line'></div>充值银行</td>
		<!--<td TITLE='银行卡尾号'><div class='line'></div>银行账号</td>-->
        <td><div class='line'></div>实际充值金额</td>
        <td><div class='line'></div>充值手续费</td>
		<td><div class='line'></div>上分金额</td>
...
<?php
  if($total==0){
?>
    	<tr align="center"><td colspan="6" class='no-records'><span>暂无数据</span></td></tr>
<?php
  }else{
...
?>
    <tr align="center">
    	<td><a TITLE='查看详情' href='./account_saveinfo.php?id=<?=$row['dan']?>&check=400'><?=$row['dan']?></a></td>
    	<td><?=$row['adddate']?></td>
		<td><?=$row['bank']?></td>
		<!--<td></td>-->
    	<td ><?=$row['money']?></td>
		...
		<td ><?=$row['rmoney']?></td>
    	<td id="status">
    		<?php if($row['zt']==1){echo "<font color=#669900>充值成功</font>";}else if($row['zt']==2){echo "<font color=#ff0000>充值失败</font>";}else if($row['zt']==0){echo "<font color=#888888>等待处理</font>";}else if($row['zt']==3){echo "<font color=#888888>用户撤消</font>";}?>
           </td>
        <td><a TITLE='查看详情' href='./account_saveinfo.php?id=<?=$row['id']?>&check=400'>详情</a></td>
    </tr>
<?php }}?>
