...
//				alert("zzz1");
			...
			$.fn.lt_opentimer=function(start,end){
				...
				function _getcode(issue){
					$.ajax({type:"POST",url:$.lt_ajaxurl,data:"lotteryid="+$.lt_lottid+"&flag=gethistory&issue="+issue,success:function(data){
						...
						$.lt_open_status=true;
						var codebox=$("div[name='historycode']");
						...
						if(data.code[$i]=="*"){
							...
							$(codebox[$i]).attr("class","gr_s gr_s"+data.code[$i])}$i--},700);
							$("#lt_opentimebox").hide();
			...
			var opentimerno=window.setInterval(
				function(){
					if($.lt_time_open<=0){
						...
						$.each($("div[name='historycode']"),function(i,n){
							if($(this).attr("class")!=""){
								$(this).attr("name","historycode")}
						});
		...
		$.ajax({type:"POST",URL:$.lt_ajaxurl,data:"lotteryid="+$.lt_lottid+"&flag=read",success:function(data){
			...
			if(data=="empty"){
				...
				window.location.href="./help_security.php";
				return false
			}
			eval("data="+data);
...
//				alert(data.issue);
//				alert($.lt_issues.today.shift().issue);
