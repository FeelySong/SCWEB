...
if (strpos($_SESSION['admin_flag'],'43') ){}else{ 
...
if($_GET['act']=="del"){
	...
	echo "<script>window.location.href='account_prize.php';</script>"; 
	exit;
}
...
if($_GET['act']=="cd"){
	...
	if($row['tag']!="撤消派奖"){
		mysql_query("update ssc_record set tag='撤消派奖' where dan='".$_GET['dan']."'");	

		$sqlb="select * from ssc_record where dan='".$_GET['dan']."' and types='12'";
		...
		$sqla="insert into ssc_record set lotteryid='".$rowb['lotteryid']."', lottery='".$rowb['lottery']."', dan='".$dan1."', dan1='".$rowb['dan1']."', dan2='".$rowb['dan2']."', uid='".$rowb['uid']."', username='".$rowb['username']."', issue='".$rowb['issue']."', types='16', mid='".$rowb['mid']."', mode='".$rowb['mode']."', zmoney=".$rowb['smoney'].",leftmoney=".$lmoney.", cont='".$rowb['cont']."', regtop='".$rowb['regtop']."', regup='".$rowb['regup']."', regfrom='".$rowb['regfrom']."', adddate='".date("Y-m-d H:i:s")."'";
	//			echo $sqla;
...
//		mysql_query("Delete from ssc_record where id=".$_GET['id']);
		...
		echo "<script>alert('该单不能被处理！');window.location.href='account_prize.php';</script>"; 
		exit;
	}
}
...
$urls="starttime=".$starttime."&endtime=".$endtime."&lotteryid=".$lotteryid."&methodid=".$methodid."&mode=".$mode."&uname=".$uname."&username=".$username_mem."&dan=".$dan;
...
<br>
	<table width="95%" border="0" cellpadding="0" cellspacing="0" class="top_list">
      <tr>
        <td class="top_list_td icons_a4">　　您现在的位置是：业务流水 &gt; 中奖记录</td>
      </tr>
    </table>
    <br>
...
<br>
	...
	<table width="95%" border="1" cellpadding="0" cellspacing="0" bordercolor="#DDDDDD" class="t_list">
		...
		?></td>
              ...
              <td><a href="?act=cd&dan=<?=$row['dan']?>" onClick="return confirm('确认要撤消派奖吗?');">撤消派奖</a> <a href="?act=del&id=<?=$row['id']?>" onClick="return confirm('确认要删除吗?');">删除</a></td>
  </tr>
 		<?php
		}
		?>
