...
if (strpos($_SESSION['admin_flag'],'a2') ){}else{ 
...
if($_GET['act']=="dels"){       //批量删除   
	...
	$sql="delete from ssc_managerlogin where id in ($ids)";  
	mysql_query($sql);  
}

if($_GET['act']=="del"){
	mysql_query("Delete from ssc_managerlogin where id=".$_GET['id']);
	echo "<script>window.location.href='admin_logs.php';</script>"; 
	exit;
}
...
$sql="select * from ssc_managerlogin where 1=1".$s1;
$rs = mysql_query($sql);
$total = mysql_num_rows($rs);

$sql="select * from ssc_managerlogin where 1=1".$s1." limit $page2,$pagesize";
...
<br>
	<table width="95%" border="0" cellpadding="0" cellspacing="0" class="top_list">
      <tr>
        <td class="top_list_td icons_a10">　　您现在的位置是：权限管理 &gt; 登陆日志</td>
      </tr>
    </table>
<br>
	...
	<table width="95%" border="1" cellpadding="0" cellspacing="0" bordercolor="#DDDDDD" class="t_list">
        <tr>
          ...
          <td class="t_list_caption">用户名</td>
            ...
            <td class="t_list_caption">客户端</td>
