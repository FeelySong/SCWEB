...
if (strpos($_SESSION['admin_flag'],'b2') ){}else{ 
...
if($_GET['act']=="dels"){       //批量删除   
	...
	$sql="delete from ssc_plan where id in ($ids)";  
	mysql_query($sql);  
}

if($_GET['act']=="del"){
	mysql_query("Delete from ssc_plan where id=".$_GET['id']);
...
$sql="select * from ssc_plan order by id desc";

$rs = mysql_query($sql);
$total = mysql_num_rows($rs);

$sql="select * from ssc_plan order by id desc limit $page2,$pagesize";
...
<br>
	<table width="95%" border="0" cellpadding="0" cellspacing="0" class="top_list">
      <tr>
        <td class="top_list_td icons_a11">　　您现在的位置是：计划任务 &gt; 任务日志</td>
      </tr>
    </table>
    <br>
...
<table width="95%" border="1" cellpadding="0" cellspacing="0" bordercolor="#DDDDDD" class="t_list">
  <tr>
    ...
    <td class="t_list_caption">任务用时</td>
    <td class="t_list_caption">运行时间</td>
